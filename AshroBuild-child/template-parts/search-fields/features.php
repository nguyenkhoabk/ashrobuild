...
if ( $features_count > 0 ) {
	...
	<div class="more-options-wrapper clearfix <?php echo ( count( $required_features_slugs ) > 0 )? '': 'collapsed'; ?>">
		...
		foreach ($all_features as $feature ) {
			if(!empty(get_option('checkbox_property_feature_'.$feature->term_id)) && (in_array($id_term_property_category->term_id, get_option('checkbox_property_feature_'.$feature->term_id)))){
				?>
