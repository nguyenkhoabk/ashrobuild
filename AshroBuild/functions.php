 ...
 */
define( 'INSPIRY_THEME_VERSION', '2.6.4' );
...
if ( ! function_exists( 'inspiry_google_fonts' ) ) :
	...
	function inspiry_google_fonts() {
		 ...
		 * Heading Font
		 */
		if ( !empty( $inspiry_heading_font ) && ( $inspiry_heading_font !== "Default" ) ) {
		 ...
		 * Secondary Font
		 */
		if ( !empty( $inspiry_secondary_font ) && ( $inspiry_secondary_font !== "Default" ) ) {
		 ...
		 * Body Font
		 */
		if ( !empty( $inspiry_body_font ) && ( $inspiry_body_font !== "Default" ) ) {
    ...
    function load_theme_scripts() {
        if ( ! is_admin() ) {
	        ...
	        if ( 'true' == get_option( 'inspiry_optimise_js' ) && ! class_exists( 'iHomefinderAdmin' ) ) {
		        ...
		        ( ! class_exists( 'iHomefinderAdmin' ) ) ? wp_enqueue_script( 'bootstrap' ) : false;
	        }
            ...
            if ( google_map_needed() ) {
	            ...
	            wp_enqueue_script(
		            'google-map-oms',
		            esc_url_raw( $js_directory_uri . 'oms.min.js' ),
		            array( 'google-map-api' ),
		            '0.3.3'
	            );
	...
	function inspiry_google_maps_api_notice() {
		...
		if ( empty( $google_maps_api_key ) ) {
			...
			<div class="notice notice-warning is-dismissible">
				<p>
					<a href="http://realhomes.inspirythemes.biz/doc/#google-maps-api-key" target="_blank"><?php esc_html_e( 'Google Maps API key', 'framework' ); ?></a> <?php esc_html_e( 'is missing. You have to add it in settings given under', 'framework' ); ?>
